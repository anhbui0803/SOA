...
from .models import Category, Group, Location, Bus, Schedule, Booking, BusBrand
...
class SaveGroup(forms.ModelForm):
    name = forms.CharField(max_length="250")
    discount = forms.FloatField(min_value=0,max_value=100)
    status = forms.ChoiceField(choices=[('1','Active'),('2','Inactive')])
    ...
    def clean_name(self):
        ...
        try:
            if int(id) > 0:
                group = Group.objects.exclude(id=id).get(name = name)
            else:
                group = Group.objects.get(name = name)
        except:
            return name
            # raise forms.ValidationError(f"{name} Category Already Exists.")
        raise forms.ValidationError(f"{name} Group Already Exists.")